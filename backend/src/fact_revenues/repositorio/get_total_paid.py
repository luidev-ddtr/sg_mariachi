...
def get_total_paid(reservation_id: str, conn: Conexion) -> float:
    """
    Obtiene la suma total (acumulado) de los pagos registrados en fact_revenues
    ...
    query = "SELECT SUM(FACT_PaymentAmount) FROM fact_revenue WHERE DIM_ReservationId = %s"
